...
# take in raw data
data_rose1 = np.loadtxt('avgrose-10-10-1')
data_rose2 = np.loadtxt('avgrose-10-10-4')
data_rose3 = np.loadtxt('avgrose-10-10-8')

length = len(data_rose1)
half_length = int(length/2)
# make complex numpy array with data
complex_data_rose1 = np.empty(half_length, dtype=complex)
complex_data_rose1.real = data_rose1[:half_length]
complex_data_rose1.imag = data_rose1[half_length:]

# make complex numpy array with data
complex_data_rose2 = np.empty(half_length, dtype=complex)
complex_data_rose2.real = data_rose2[:half_length]
complex_data_rose2.imag = data_rose2[half_length:]

# make complex numpy array with data
complex_data_rose3 = np.empty(half_length, dtype=complex)
complex_data_rose3.real = data_rose3[:half_length]
complex_data_rose3.imag = data_rose3[half_length:]

# compute fourier transform
fourier_rose1 = np.fft.fft(complex_data_rose1)
# sample rate is 62.5e6 Hz, spacing is inverse
sample_spacing = 1.6e-8
fourier_axis_rose1 = np.fft.fftfreq(n=len(complex_data_rose1), d=sample_spacing)
# fourier filter
fourier_rose1[0] = 0

# compute fourier transform
fourier_rose2 = np.fft.fft(complex_data_rose2)
# sample rate is 62.5e6 Hz, spacing is inverse
sample_spacing = 6.4e-8
fourier_axis_rose2 = np.fft.fftfreq(n=len(complex_data_rose2), d=sample_spacing)
# fourier filter
fourier_rose2[0] = 0

# compute fourier transform
fourier_rose3 = np.fft.fft(complex_data_rose3)
# sample rate is 62.5e6 Hz, spacing is inverse
sample_spacing = 1.28e-7
fourier_axis_rose3 = np.fft.fftfreq(n=len(complex_data_rose3), d=sample_spacing)
# fourier filter
fourier_rose3[0] = 0



plt.plot(np.fft.fftshift(fourier_axis_rose1), np.abs(np.fft.fftshift(fourier_rose1))**2, label="62.5 MHz")

#plt.plot(np.fft.fftshift(fourier_axis_rose2), np.abs(np.fft.fftshift(fourier_rose2))**2, label="15.625 MHz")

#plt.plot(np.fft.fftshift(fourier_axis_rose3), np.abs(np.fft.fftshift(fourier_rose3))**2, label="7.8125 MHz")
